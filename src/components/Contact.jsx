/**
 * Contact section with local submit to our Express backend.
 */
...
const socialLinks = [
  ...
  {
    href: 'https://www.linkedin.com/in/dino-bajramović/',
    label: 'LinkedIn',
    icon: <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const Contact = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!validateName(name)) {
      setNameError('Please enter a valid name (letters, space, apostrophe, dash).');
      hasError = true;
    }
    ...
    if (message.trim().length > MESSAGE_LIMIT) {
      setStatus(`Message is too long. Max ${MESSAGE_LIMIT} characters.`);
      setStatusType('error');
      setSubmitting(false);
      return;
    }
